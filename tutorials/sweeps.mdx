...
1. **Define the sweep:** we do this by creating a dictionary or a [YAML file](/guides/models/sweeps/define-sweep-configuration) that specifies the parameters to search through, the search strategy, the optimization metric et all.
...
If you run a sweep within the command line, you must specify your sweep config with a [YAML file](/guides/models/sweeps/define-sweep-configuration).
</Note>

### Pick a search method

First, specify a hyperparameter search method within your configuration dictionary. [There are three hyperparameter search strategies to choose from: grid, random, and Bayesian search](/guides/models/sweeps/define-sweep-configuration/sweep-config-keys/#method).
...
For a full list of configuration options, see [Sweep configuration options](/guides/models/sweeps/define-sweep-configuration/sweep-config-keys/). 
...
For more information on how to create W&B Sweeps in a terminal, see the [W&B Sweep walkthrough](/guides/models/sweeps/walkthrough).
...
* [`run.config`](/guides/models/track/config): Pass sweep configuration with the hyperparameters you want to experiment with.
