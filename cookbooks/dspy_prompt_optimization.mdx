...
This tutorial demonstrates how you can improve the performance of your LLM workflow implemented  on the **causal judgement task** from the BIG-bench Hard benchmark and evaluate your prompting strategies. You use [DSPy](https://dspy-docs.vercel.app/) for implementing your LLM workflow and optimizing your prompting strategy. You also use [Weave](https://docs.wandb.ai/guides/weave) to track your LLM workflow and evaluate your prompting strategies.

## Installing the Dependencies

You need the following libraries for this tutorial:

- [DSPy](https://dspy-docs.vercel.app/) for building the LLM workflow and optimizing it.
- [Weave](https://docs.wandb.ai/guides/weave) to track your LLM workflow and evaluate your prompting strategies.
...
Since we'll be using [OpenAI API](https://openai.com/index/openai-api/) as the LLM Vendor, you also need an OpenAI API key. You can [sign up](https://platform.openai.com/signup) on the OpenAI platform to get your own API key.
...
In this tutorial, you use a metadata class inherited from [`weave.Object`](/guides/tracking/objects) to manage your metadata.
...
You load this dataset from HuggingFace Hub, split into training and validation sets, and [publish](https://docs.wandb.ai/guides/weave/core-types/datasets) them on Weave, this lets you version the datasets, and also use [`weave.Evaluation`](https://docs.wandb.ai/guides/weave/core-types/evaluations) to evaluate your prompting strategy.
...
You use the [`dspy.OpenAI`](https://dspy-docs.vercel.app/api/language_model_clients/OpenAI) abstraction to make LLM calls to [GPT3.5 Turbo](https://platform.openai.com/docs/models/gpt-3-5-turbo).
...
To start, test our LLM workflow, i.e., the `CausalReasoningModule` on an example from the causal reasoning subset of Big-Bench Hard.
...
Now that you have a baseline prompting strategy, let's evaluate it on the validation set using [`weave.Evaluation`](https://docs.wandb.ai/guides/weave/core-types/evaluations) on a simple metric that matches the predicted answer with the ground truth. Weave takes each example, passes it through your application and scores the output on multiple custom scoring functions. By doing this, you'll have a view of the performance of your application, and a rich UI to drill into individual outputs and scores.

First, you need to create a simple weave evaluation scoring function that tells whether the answer from the baseline module's output is the same as the ground truth answer or not. Scoring functions need to have a `model_output` keyword argument, but the other arguments are user defined and are taken from the dataset examples. It only takes the necessary keys by using a dictionary key based on the argument name.
...
Next, you can simply define the evaluation and run it.
...
Now, that you have a baseline DSPy program, let us try to improve its performance for causal reasoning using a [DSPy teleprompter](https://dspy-docs.vercel.app/docs/building-blocks/optimizers) that can tune the parameters of a DSPy program to maximize the specified metrics. In this tutorial, you use the [BootstrapFewShot](https://dspy-docs.vercel.app/api/category/optimizers) teleprompter.
...
Now that you have the optimized program (the optimized prompting strategy), let's evaluate it once again on the validation set and compare it with your baseline DSPy program.
...
In this tutorial, you learned how to use DSPy for prompt optimization alongside using Weave for tracking and evaluation to compare the original and optimized programs.