...
- [Weave](https://docs.wandb.ai/guides/weave) to track the LLM workflow and evaluate the prompting strategies.
...
Since this tutorial uses [OpenAI API](https://openai.com/index/openai-api/) as the LLM Vendor, you will also need an OpenAI API key. You can [sign up](https://platform.openai.com/signup) on the OpenAI platform to get your own API key.
...
In this tutorial, you use a metadata class inherited from [`weave.Object`](/guides/tracking/objects) to manage the metadata.
...
This tutorial loads the dataset from HuggingFace Hub, splits it into training and validation sets, and [publishes](https://docs.wandb.ai/guides/weave/core-types/datasets) them on Weave, which lets you version the datasets, and also use [`weave.Evaluation`](https://docs.wandb.ai/guides/weave/core-types/evaluations) to evaluate your prompting strategy.
...
This tutorial uses the [`dspy.OpenAI`](https://dspy-docs.vercel.app/api/language_model_clients/OpenAI) abstraction to make LLM calls to [GPT3.5 Turbo](https://platform.openai.com/docs/models/gpt-3-5-turbo).
...
This section tests the LLM workflow, i.e., the `CausalReasoningModule` on an example from the causal reasoning subset of Big-Bench Hard.
...
Now, that you have a baseline DSPy program, this tutorial improves its performance for causal reasoning using a [DSPy teleprompter](https://dspy-docs.vercel.app/docs/building-blocks/optimizers) that can tune the parameters of a DSPy program to maximize the specified metrics. This tutorial uses the [BootstrapFewShot](https://dspy-docs.vercel.app/api/category/optimizers) teleprompter.
...
Now that you have the optimized program (the optimized prompting strategy), this tutorial evaluates it once again on the validation set and compares it with the baseline DSPy program.
