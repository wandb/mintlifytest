...
# Then, when you use the `@weave.op` decorator, we pass these processing functions as arguments to the decorator:
@weave.op(
...
!pip install Faker                          # this tutorial will use Faker to replace PII data with fake data
!pip install weave                          # To leverage Traces
!pip install set-env-colab-kaggle-dotenv -q # for env var
!pip install anthropic                      # to use sonnet
!pip install cryptography                   # to encrypt the data
```
...
To start, test the function with a sample text:
...
Let's consolidate your code into a single class and expand the list of entities to include the additional ones identified earlier.
...
In the following examples, you will integrate the PII redaction and anonymization methods into Weave Models and preview the results in Weave Traces.

First, this tutorial will create a [Weave Model](https://wandb.github.io/weave/guides/core-types/models). A Weave Model is a combination of information like configuration settings, model weights, and code that defines how the model operates.

In the model, you will include your predict function where the Anthropic API will be called. Anthropic's Claude Sonnet is used to perform sentiment analysis while tracing LLM calls using [Traces](https://wandb.github.io/weave/quickstart). Claude Sonnet will receive a block of text and output one of the following sentiment classifications: _positive_, _negative_, or _neutral_. Additionally, you will include the postprocessing functions to ensure that your PII data is redacted or anonymized before it is sent to the LLM.
...
In the simplest case, you can use regex to identify and redact PII data from the original text.
...
Next, you will use Presidio to identify and redact PII data from the original text.
...
class sentiment_analysis_model(weave.Model):
    ...
    async def predict(self, encrypted_input: EncryptedSentimentAnalysisInput) -> dict:
        ...
        decrypted_text = encrypted_input.decrypt() # You use the custom class to decrypt the text
