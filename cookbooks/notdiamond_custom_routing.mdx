...
1. A set of LLM prompts: Prompts must be strings and should be representative of the prompts used in the application.
...
You start by downloading the dataset you have prepared for this example, then parsing LLM responses into EvaluationResults for each model.
...
![Create an API key](/guides/integrations/imgs/notdiamond/api-keys.png)
...
![Check on router training progress](/guides/integrations/imgs/notdiamond/router-preferences.png)
...
![Weave UI for custom routing](/guides/integrations/imgs/notdiamond/weave-trace.png)
...
In the UI you see that the Not Diamond "meta-model" outperforms the best-performing model by routing prompts to other models with higher likelihood of answering the prompt accurately.

![Evaluating Not Diamond](/guides/integrations/imgs/notdiamond/evaluations.png)