...
In this cookbook you explore the lower level Weave Python API to extract data from a CSV file and import it into Weave to drive insights and rigorous evaluations.
...
You therefore use `conversation_id` as the parent identifier, and the `turn_index` as the child identifier to provide you complete conversation logging.

Ensure to modify the variables as needed.

# Set up the environment

You install and import all needed packages.
You set `WANDB_API_KEY` in your env so that you may easily login with `wandb.login()` (this should be given to the colab as a secret).

You set the name of the file you upload to colab in `name_of_file` and set the project in W&B you want to log this into in `name_of_wandb_project`.
...
You then fetch a weave client by calling `weave.init()`
...
You load the data into a Pandas dataframe, and ensure you sort it by the `conversation_id` and `turn_index` to ensure the parents and childs are correctly ordered.
...
You now iterate through your pandas DF:
- You create a parent call for every `conversation_id`
- You iterate through the turn array to create child calls sorted by their `turn_index`
...
- A weave call is `created` and `finished` as these are meant to be tracked real time. Because this is an after-the-fact import, you create and finish once your objects are defined and tied to one another.
...
for _, row in result_df.iterrows():
    ...
    # you are now creating a "call" with the weave_client we defined before
    ...
    for item in row["conversation_data"]:
        ...
        # You create a call again here to be categorized under the conversation
        call = weave_client.create_call(
            # We qualify a single conversation trace as a "Turn"
            op="Turn",
            # You provide all inputs of the turn, including RAG 'ground_truth'
            inputs={
            ...
            parent=parent_call,
            # You provide it a name to be id'ed by in Weave
    ...
    # Now that you have logged all its children, we also finish the parent call
...
Once our traces are in Weave and you have an understanding on how the conversations are looking, we may want to later on export them to another process to run Weave Evaluations
...
# You create a query filter that brings you all your Conversation objects
...
for single_conv in conversation_traces:
  ...
  # Once we've identified a converation to have used RAG, we add it to the dataset
  if is_rag:
    inputs = []
    ground_truths = []
    answers = []
...
# With the dataset rows created, you create the Dataset object and
