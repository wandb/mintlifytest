...
Next, load the required API keys for OpenAI and Weave. Here, you use set_env which is compatible with google colab's secret keys manager, and is an alternative to colab's specific `google.colab.userdata`. See: [here](https://pypi.org/project/set-env-colab-kaggle-dotenv/) for usage instructions.
...
Now you will setup a call to OpenAI's completions endpoint with audio modality enabled. First create the OpenAI client and initiate a Weave project.
...
Now you will define your OpenAI completions request and add your Weave decorator (op).
...
```python
# Get device list from pyaudio so you can configure the next cell
p = pyaudio.PyAudio()
...
OAI_SAMPLE_RATE = (
    24000  # OAI Sample Rate is 24kHz, you need this to play or save assistant audio
)
...
You use a pyaudio input stream with a handler linked to the `send_audio` method of the RTAudio model. The stream is returned to the main thread so it can be safely exited upon program completion.
