...
To start, set up a simple custom model, that calculates its own token usage, and stores that in weave.
...
You set the project in W&B you want to log this into in `name_of_wandb_project`.
...
class YourModel(Model):
    ...
    # This is a custom op that you are defining
    ...
    def custom_model_generate(self, input_data: str) -> dict:
        ...
        # You return a dictionary with the usage counts, model name, and the output
    ...
    # In your predict function you call your custom generate function, and return the output.
    @weave.op()
...
Here you add a custom cost, and now that you have a custom cost, and your calls have usage, you can fetch the calls with `include_cost` and your calls with have costs under `summary.weave.costs`.

```python
model = YourModel(attribute1="Hello", attribute2=1)
model.predict("world")

# You then add a custom cost to your project
weave_client.add_cost(
    llm_id="your_model_name", prompt_token_cost=0.1, completion_token_cost=0.2
)

# You can then query for the calls, and with include_costs=True
# you receive the costs back attached to the calls
