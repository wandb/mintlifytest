...
| `WANDB_JOB_TYPE` | Specify the job type, like "training" or "evaluation" to indicate different types of runs. See [grouping](/guides/runs/grouping/) for more info. |
...
| `WANDB_RUN_GROUP` | Specify the experiment name to automatically group runs together. See [grouping](/guides/runs/grouping/) for more info. |
