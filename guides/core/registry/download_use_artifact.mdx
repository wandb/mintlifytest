...
The `.use_artifact()` method both creates a [run](/guides/models/track/runs/) and marks the artifact you download as the input to that run. 
...
See [`use_artifact`](/ref/python/experiments/run.md#use_artifact) and [`Artifact.download()`](/ref/python/experiments/artifact.md#download) in the API Reference for parameters and return type.
