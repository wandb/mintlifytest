...
Configure Slack destinations where your team's [automations](/guides/automations/) can send notifications when an event occurs in a registry or a project, such as when a new artifact is created or when a run metric meets a defined threshold. Refer to [Create a Slack automation](/guides/automations/create-automations/slack).
...
See the [pricing page](https://wandb.ai/site/pricing) for more information on W&B plans. You can download all your data at any time, either using the dashboard UI or the [Export API](/ref/python/public-api).
