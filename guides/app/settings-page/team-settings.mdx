...
For more information about how to set up and manage alerts, see [Send alerts with `wandb.Run.alert()`](/guides/models/evaluate-models/).
...
## Webhooks
Configure webhooks that your team's [automations](/guides/automations/) can run when an event occurs in a Registry or a project, such as when a new artifact is created or when a run metric meets a defined threshold. Refer to [Create a webhook automation](/guides/automations/create-automations/slack/).
...
The **Storage** section describes the cloud storage bucket configuration that is being used for the team's data. For more information, see [Secure Storage Connector](/guides/app/settings-page/teams/#secure-storage-connector) or check out our [W&B Server](/guides/hosting/data-security/secure-storage-connector) docs if you are self-hosting.