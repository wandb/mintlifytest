...
<Note>
The term "type" refers to the artifact object's type. When you create an artifact object ([`wandb.Artifact`](/ref/python/experiments/artifact)), or log an artifact ([`wandb.init.log_artifact`](/ref/python/experiments/run#log_artifact)), you specify a type for the `type` parameter. 
...
Programmatically link an artifact version to a collection with [`wandb.init.Run.link_artifact()`](/ref/python/experiments/run#link_artifact).
