...
<Note>
Manual linking a model to the model registry is useful for one-off models. However, it is often useful to [programmatically link model versions to the model registry](/guides/registry/model_registry/link-model-version/#programmatically-link-a-model).
