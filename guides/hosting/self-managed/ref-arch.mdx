...
### Kubernetes
The W&B Server application is deployed as a [Kubernetes Operator](kubernetes-operator/) that deploys multiple pods. For this reason, W&B requires a Kubernetes cluster with:
...
To learn about air-gapped deployments, refer to [Kubernetes operator for air-gapped instances](kubernetes-operator/operator-airgapped).
...
Terraform is the recommended way to deploy W&B for production. Using Terraform, you define the required resources, their references to other resources, and their dependencies. W&B provides Terraform modules for the major cloud providers. For details, refer to [Deploy W&B Server within self managed cloud accounts](/guides/hosting/hosting-options/self-managed.md#deploy-wb-server-within-self-managed-cloud-accounts).
