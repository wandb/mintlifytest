...
<Warning>
1. Before you initialize a sweep, make sure you have a sweep configuration defined either in a YAML file or a nested Python dictionary object in your script. For more information, see [Define sweep configuration](/guides/models/sweeps/define-sweep-configuration).
2. Both the W&B Sweep and the W&B Run must be in the same project. Therefore, the name you provide when you initialize W&B ([`wandb.init()`](/ref/python/functions/init)) must match the name of the project you provide when you initialize a W&B Sweep ([`wandb.sweep()`](/ref/python/functions/sweep)).
</Warning>
...
The [`wandb.sweep()`](/ref/python/functions/sweep) function returns the sweep ID. The sweep ID includes the entity name and the project name. Make a note of the sweep ID.
</Tab>
<Tab title="CLI">
