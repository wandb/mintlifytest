...
To fork from a rewound run, use the [`fork_from`](/guides/models/track/runs/forking) argument in `wandb.init()` and specify the source run ID and the step from the source run to fork from:
