...
In some applications you want to create custom `Scorer` classes - where for example a standardized `LLMJudge` class should be created with specific parameters (e.g. chat model, prompt), specific scoring of each row, and specific calculation of an aggregate score.
...
# This dataset has "input_text" but the model expects "question"
examples = [
...
In this example, the dataset contains examples with an `input_text` field, but our evaluation function expects a `question` parameter. The `preprocess_example` function transforms each example by renaming the field, allowing the evaluation to work correctly.
...
You are continuously improving our integrations with third-party services and libraries.
...
See our [Using HuggingFace datasets in evaluations cookbook](/cookbooks/hf_dataset_evals) for the current approach.

### Saved views 

You can save your Evals table configurations, filters, and sorts as _saved views_ for quick access to your preferred setup. You can configure and access saved views via the UI and the Python SDK. For more information, see [Saved Views](/guides/tools/saved-views.mdx).
