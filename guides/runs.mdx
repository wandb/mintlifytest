...
* Logging data or a model as a [W&B Artifact](/guides/core/artifacts/)
* [Downloading a W&B Artifact](/guides/core/artifacts/download-and-use-an-artifact)


W&B stores runs that you create into [*projects*](/guides/models/track/project-page). You can view runs and their properties within the run's project workspace on the W&B App. You can also programmatically access run properties with the [`wandb.Api.Run`](/ref/python/experiments/run) object.
...
As another example, during a [sweep](/guides/models/sweeps/), W&B explores a hyperparameter search space that you specify. W&B implements each new hyperparameter combination that the sweep creates as a unique run.
...
If you [group runs](grouping) into experiments, you can move a run into or out of a group or from one group to another.
...
You can also rename a run from a run set in a [report](/guides/core/reports/edit-a-report):
...
* **Git repository**: The git repository associated with the run. You must [enable git](/guides/models/app/settings-page/user-settings.md#personal-github-integration) to view this field.
...
* **Config**: List of config parameters saved with [`wandb.Run.config`](/guides/models/track/config).
* **Summary**: List of summary parameters saved with [`wandb.Run.log()`](/guides/models/track/log/). By default, W&B sets this value to the last value logged.
...
<Note>
The **Group by** feature is distinct from a [run's run group](grouping). You can group runs by run group. To move a run to a different run group, refer to [Assign a group or job type to a run](#assign-a-group-or-job-type-to-a-run).
</Note>
</Tab>
</Tabs>
...
### Artifacts tab
The **Artifacts** tab lists the input and output [artifacts](/guides/core/artifacts/) for the specified run.

<Frame>
    <img src="/images/app_ui/artifacts_tab.png" alt="Run artifacts tab"  />
</Frame>

View [example artifact graphs](/guides/core/artifacts/explore-and-traverse-an-artifact-graph).
