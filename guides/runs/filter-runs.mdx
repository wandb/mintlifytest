...
See Customize run colors for more information on how to [edit, randomize, and reset run colors](/guides/runs/run-colors/).
...
If regex doesn't provide you the desired results, you can make use of [tags](/guides/runs/filter-runs/#filter-runs-with-tags) to filter out the runs in Runs Table. Tags can be added either on run creation or after they're finished. Once the tags are added to a run, you can add a tag filter as shown in the gif below.
