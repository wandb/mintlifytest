...
W&B Registry is a curated central repository of [W&B Artifact](/guides/core/artifacts/) versions within your organization. Users who [have permission](./configure_registry) within your organization can [download and use artifacts](./download_use_artifact), share, and collaboratively manage the lifecycle of all artifacts, regardless of the team that user belongs to.

You can use the Registry to [track artifact versions](./link_version), audit the history of an artifact's usage and changes, ensure governance and compliance of your artifacts, and [automate downstream processes such as model CI/CD](/guides/core/automations/).
...
- Track an [artifact’s lineage](/guides/core/registry/lineage) and audit the history of changes.
- [Automate](/guides/core/automations/) downstream processes such as model CI/CD.
...
To add an artifact to a registry, you first log a [specific artifact version to W&B](/guides/core/artifacts/create-a-new-artifact-version). Each time you log an artifact, W&B automatically assigns a version to that artifact. Artifact versions use 0 indexing, so the first version is `v0`, the second version is `v1`, and so on. 
...
As an example, the proceeding code example shows how to log and link a model artifact called "my_model.txt" to a collection named "first-collection" in the [core registry](./registry_types):
