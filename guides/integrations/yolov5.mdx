...
Simply by installing `wandb`, you'll activate the built-in W&B [logging features](/guides/track/log/): system metrics, model metrics, and media logged to interactive [Dashboards](/guides/track/workspaces/).
...
* If you pass a number to `--save_period`, W&B saves a [model version](/guides/registry/) at the end of every `save_period` epochs. The model version includes the model weights and tags the best-performing model in the validation set.
