...
| `model_log_interval`   | `Optional int`. `None` by default. If set, enables [model versioning](/guides/registry/) with [Artifacts](/guides/artifacts/). Pass in the number of steps to wait between logging model checkpoints. `None` by default. |
...
When training begins, a link to your training run's [W&B page](/guides/runs/) will be output which will take you to this run's experiment tracking [dashboard](/guides/track/workspaces/) in the W&B web UI.