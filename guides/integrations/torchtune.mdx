...
You can also pass any other `kwargs` as you would to the [wandb.init()](/ref/python/functions/init) method. For example, if you are working on a team, you can pass the `entity` argument to the `WandBLogger` class to specify the team name.
...
If you want to save the model checkpoints to [W&B Artifacts](/guides/core/artifacts/), the simplest solution is to override the `save_checkpoint` functions inside the corresponding recipe. 
