...
An automation can start when a specific event occurs within a project or registry. This page describes the events that can trigger an automation within each scope. Learn more about automations in the [Automations overview](/guides/core/automations/) or [Create an automation](create-automations/).

## Registry
This section describes the scopes and events for an automation in a [Registry](/guides/core/registry/).
...
Learn more about [creating automations](create-automations/).

### Scopes
You can create a Registry automation at these scopes:
- [Registry](/guides/core/registry/) level: The automation watches for the event taking place on any collection within a specific registry, including collections added in the future.
...
## Project
This section describes the scopes and events for an automation in a [project](/guides/models/track/project-page).
...
### Run events
An automation can be triggered by a change in a [run's status](/guides/models/track/runs/#run-states) or a change in a [metric value](/guides/models/track/log/#what-data-is-logged-with-specific-wb-api-calls).
...
Trigger a workflow when a run changes its [status](/guides/models/track/runs/_index.md#run-states) to **Running**, **Finished**, or **Failed**. Optionally, you can further limit the runs that can trigger an automation by filtering by the user that started a run or the run's name.
...
## Next steps
- [Create a Slack automation](create-automations/slack)
- [Create a webhook automation](create-automations/webhook)
