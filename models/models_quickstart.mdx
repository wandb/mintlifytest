...
Use the W&B Python SDK (`wandb.sdk`) to interact with W&B during training. Log the loss using [`wandb.Run.log()`](/ref/python/experiments/run/#method-runlog), then save the trained model as an artifact using [`wandb.Artifact`](/ref/python/experiments/artifact) before finally adding the model file using [`Artifact.add_file`](/ref/python/experiments/artifact#add_file).
