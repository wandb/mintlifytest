...
To disable logging of [system metrics](/ref/system-metrics), set `_disable_stats` to `True`:
...
- Set the `--host` flag of [`wandb login`](/ref/cli/wandb-login) to the Server URL.
</Accordion>
</Update>
...
To configure the frequency to log [system metrics](/ref/system-metrics), set `_stats_sampling_interval` to a number of seconds, expressed as a float. Default: `10.0`.
...
See [Multiple runs per process](/guides/runs/multiple-runs-per-process)
...
These examples show logging losses a couple of different ways using [`wandb.Run.log()`](/ref/python/experiments/run/#method-runlog/).
...
1. Choose a new value for **Project visibility**:
    ...
    - **Public**: Anyone can view the project, but only your team can edit it.
        ...
        <Note>
        If your W&B admins have turned off **Public** visibility, you cannot choose it. Instead, you can share a view-only [W&B Report](/guides/reports/collaborate-on-reports#share-a-report), or contact your W&B organization's admins for assistance.
        </Note>
1. Click **Save**.
...
- Pass it to [`wandb.Artifact().download`](/ref/python/public-api/api):

    ```python
    wandb.Artifact().download(root="<path_to_download>")
    ```

- Set the `WANDB_ARTIFACT_DIR` [environment variable](/guides/track/environment-variables):
