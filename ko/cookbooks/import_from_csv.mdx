...
***NOTE:*** `name_of_wandb_project`는 또한 `{team_name}/{project_name}` 형식일 수 있어 트레이스를 로깅할 팀을 지정합니다.
...
이렇게 하면 `conversation_data` 아래에 대화 턴이 배열로 있는 두 개의 열을 가진 pandas DF가 생성됩니다.
...
평가에 대해 더 알아보려면, 새로 생성된 데이터셋을 사용하여 RAG 애플리케이션을 평가하는 방법에 대한 [빠른 시작](https://weave-docs.wandb.ai/tutorial-rag)을 확인하세요!
