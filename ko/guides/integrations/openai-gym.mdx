...
저희 gym integration은 매우 가볍습니다. `gym`에서 기록된 [비디오 파일의 이름을 확인](https://github.com/wandb/wandb/blob/master/wandb/integration/gym/__init__.py#L15)하여 그에 따라 이름을 지정하거나 일치하는 항목을 찾지 못하면 `"videos"`로 대체합니다. 더 많은 제어를 원하시면 언제든지 수동으로 [비디오를 기록](/ko/guides/track/log/media/)할 수 있습니다.
