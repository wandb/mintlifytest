...
다음 코드 조각은 W&B로 에이전트를 시작하는 방법을 보여줍니다. 이미 구성 파일이 있고 W&B 스윕을 초기화했다고 가정합니다. 구성 파일을 정의하는 방법에 대한 자세한 내용은 [스윕 구성 정의](/ko/guides/sweeps/define-sweep-configuration/)을 참조하세요.
...
<Warning>
랜덤 및 베이지안 탐색은 영원히 실행됩니다. 커맨드라인, Python 스크립트 또는 [Sweeps UI](/ko/guides/sweeps/sweeps-ui/) 내에서 프로세스를 중지해야 합니다.
</Warning>
...
<Tabs>
<Tab title="Python 스크립트 또는 노트북">
먼저 스윕을 초기화합니다. 자세한 내용은 [스윕 초기화](/ko/guides/sweeps/initialize-sweeps/)을 참조하세요.
...
<Tab title="CLI">
먼저 [`wandb sweep`](/ko/ref/cli/wandb-sweep) 코맨드로 스윕을 초기화합니다. 자세한 내용은 [스윕 초기화](/ko/guides/sweeps/initialize-sweeps/)을 참조하세요.
