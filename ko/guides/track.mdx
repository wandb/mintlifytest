...
몇 줄의 코드로 기계 학습 Experiments 를 추적하세요. 그런 다음 [대화형 대시보드](/ko/guides/models/track/workspaces)에서 결과를 검토하거나, [Public API](/ko/ref/python/public-api/)를 사용하여 프로그래밍 방식으로 엑세스할 수 있도록 데이터를 Python으로 내보낼 수 있습니다.
...
위의 이미지는 여러 [runs](/ko/guides/models/track/runs/)에서 메트릭을 보고 비교할 수 있는 대시보드 의 예시를 보여줍니다.

## 작동 방식

몇 줄의 코드로 기계 학습 experiment 를 추적합니다.
1. [W&B run](/ko/guides/models/track/runs/)을 만듭니다.
2. 학습률 또는 모델 유형과 같은 하이퍼파라미터 사전을 구성 ([`run.config`](/ko/./config))에 저장합니다.
3. 정확도 및 손실과 같은 트레이닝 루프에서 시간 경과에 따른 메트릭 ([`run.log()`](/ko/guides/models/track/log/))을 기록합니다.
...
* 데이터셋 artifact 를 생성, 추적 및 사용하는 데 사용할 수 있는 W&B Python SDK 코맨드에 대한 단계별 개요는 [W&B 퀵스타트](/ko/guides/quickstart)를 참조하세요.
