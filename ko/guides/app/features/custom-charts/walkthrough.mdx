...
먼저 스크립트에 데이터를 기록합니다. 트레이닝 시작 시 설정된 단일 포인트 (예: 하이퍼파라미터)에는 [wandb.config](/ko/guides/track/config/)를 사용합니다. 시간에 따른 여러 포인트에는 [wandb.log()](/ko/guides/track/log/)를 사용하고, `wandb.Table()`을 사용하여 커스텀 2D 배열을 기록합니다. 기록된 키당 최대 10,000개의 데이터 포인트를 기록하는 것이 좋습니다.
