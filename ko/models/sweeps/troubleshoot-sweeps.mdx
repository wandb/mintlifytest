...
YAML 파일 또는 중첩된 사전 내에서 최적화할 "metric"이라는 키를 지정합니다. 이 메트릭을 반드시 로깅(`wandb.log`)해야 합니다. 또한 Python 스크립트 또는 Jupyter Notebook 내에서 스윕을 최적화하도록 정의한 _정확한_ 메트릭 이름을 사용해야 합니다. 구성 파일에 대한 자세한 내용은 [스윕 구성 정의](/ko/guides/sweeps/define-sweep-configuration/)를 참조하세요.
