...
1. 標準スイープスケジューラー: デフォルトの W&B Sweep スケジューリングエンジンを使用して [W&B Sweeps](/ja/guides/sweeps/) を制御します。`bayes`、`grid`、`random` メソッドが利用可能です。
...
スイープ設定の作成方法についての情報は、[Define sweep configuration](/ja/guides/sweeps/define-sweep-configuration) ページを参照してください。
...
W&B Sweeps についての詳細は、[Tune Hyperparameters](/ja/guides/sweeps/) チャプターを参照してください。
</Tab>
</Tabs>
