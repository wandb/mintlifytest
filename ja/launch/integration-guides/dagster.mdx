...
* [W&B Artifacts](/ja/guides/artifacts/) の使用と作成。
...
3. **W&B project**: [W&B Runs](/ja/guides/runs/) が保存されるプロジェクトの名前。
...
2. [認証ページ](https://wandb.ai/authorize) またはユーザー/チーム設定で APIキーを集めます。プロダクション環境では、そのキーを所有するために [サービスアカウント](/ja/support/service_account_useful) を使用することをお勧めします。
...
## W&B Artifacts を書き込む
続行する前に、W&B Artifacts の使用方法について十分な理解を持っていることをお勧めします。[Guide on Artifacts](/ja/guides/artifacts/) を検討してください。
...
[Run を再開可能にする](/ja/guides/runs/resuming) ことと、W&B Run ID を Dagster Run ID またはお好みの文字列に設定することの両方をお勧めします。この推奨事項に従うことで、Dagster 内でモデルをトレーニングする際に W&B メトリクスと W&B Artifacts がすべて同じ W&B Run に格納されていることが保証されます。
