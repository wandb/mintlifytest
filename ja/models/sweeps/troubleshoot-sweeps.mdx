...
YAML ファイルやネストされた辞書内で、最適化する「metric」 というキーを指定します。このメトリックをログ (`wandb.log`) することを確認してください。また、Python スクリプトや Jupyter Notebook 内で最適化するように定義した _exact_ なメトリック名を必ず使用してください。設定ファイルについての詳細は、[Define sweep configuration](/ja/guides/sweeps/define-sweep-configuration/) を参照してください。