...
[W&B レジストリアプリのグローバル検索バー](/ja/guides/core/registry/search_registry/#search-for-registry-items)を使用して、レジストリ、コレクション、アーティファクトバージョンタグ、コレクションタグ、またはエイリアスを見つけます。W&B Python SDK を使用して、特定の条件に基づいて [レジストリ、コレクション、およびアーティファクトバージョンをフィルタリング](/ja/guides/core/registry/search_registry/#query-registry-items-with-mongodb-style-queries) するために、MongoDBスタイルのクエリを使用することができます。
...
`wandb.Api().registries()` メソッドを使用するには、まず W&B Python SDK （[`wandb`](/ja/ref/python/)） ライブラリをインポートします。
```python
import wandb
...
アーティファクトオブジェクトの属性の完全なリストについては、API Reference docs の [Artifacts Class](/ja/ref/python/artifact/) を参照してください。
