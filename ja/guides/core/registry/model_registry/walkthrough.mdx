...
[`link_model`](/ja/ref/python/run.md#link_model) API を使用して、一つまたは複数のファイルを W&B run にログし、それを [W&B Model Registry](/ja/./) にリンクします。
...
オプションのパラメータに関する詳細は、API リファレンスガイドの [`link_model`](/ja/ref/python/run.md#link_model) を参照してください。
...
評価したい W&B からの[モデルバージョン](/ja/./model-management-concepts.md#model-version) をダウンロードします。`use_model` API を使用してモデルにアクセスし、ダウンロードします。
...
## モデルバージョンを昇格する
[*モデルエイリアス*](/ja/./model-management-concepts.md#model-alias) を使用して、機械学習ワークフローの次のステージに準備が整ったモデルバージョンをマークします。各登録済みモデルは 1 つまたは複数のモデルエイリアスを持つことができます。モデルエイリアスは、1 度に 1 つのモデルバージョンにのみ所属できます。
