...
<Note>
W&B の使い方をまだ学んでいる場合は、読み進める前に [実験管理](/ja/guides/track/) など、他の W&B ガイドを探索することをお勧めします。
</Note>
...
1. `WANDB_API_KEY` [環境変数](/ja/guides/track/environment-variables/) にあなたの API キーを設定します。
...
設定ファイルを定義する方法の詳細については、[実験を設定する](/ja/guides/track/config/)を参照してください。

### W&B へのログ記録

#### メトリクスのログ記録

キーがメトリクス名となる辞書を作成し、この辞書オブジェクトを [`run.log`](/ja/guides/track/log/) に渡します：
...
[`run.log` の詳細を学ぶ](/ja/guides/track/log/)。
...
[メディア、オブジェクト、プロットなどのログ記録について詳しく学ぶ](/ja/guides/track/log/)。
...
詳細については [分散トレーニング実験のログ記録](/ja/guides/track/log/distributed-training/) を参照してください。
...
カスタムエイリアスの作成方法については [カスタムエイリアスを作成する](/ja/guides/artifacts/create-a-custom-alias/) を参照してください。
...
詳細については、[Artのダウンロードと使用](/ja/guides/artifacts/download-and-use-an-artifact/)を参照してください。

### ハイパーパラメータのチューニング

ライブラリが W&B ハイパーパラメータチューニングを活用したい場合、[W&B Sweeps](/ja/guides/sweeps/) もライブラリに追加できます。
