---
- title: AutoGen
- description: Weaveを使用してAutoGenエージェントとマルチエージェントシステムを追跡・監視する
---

{/* TODO: Add a Colab notebook for this guide with a badge link here. */}

AutoGenはMicrosoftが開発したAIエージェントとアプリケーションを構築するためのフレームワークです。複雑なマルチエージェントシステムの作成を簡素化し、会話型AI(`AgentChat`)、コアマルチエージェント機能(`Core`)、および外部サービスとの統合(`Extensions`)を提供します。AutoGenはまた`Studio`コードなしでエージェントをプロトタイピングするための を提供します。詳細については、[公式AutoGenドキュメント](https://microsoft.github.io/autogen/stable//index.html)をご覧ください。

<Note>
  このガイドでは、[AutoGen](https://microsoft.github.io/autogen/stable/index.html)の基本的な理解があることを前提としています。
</Note>

Weaveは[AutoGen](https://microsoft.github.io/autogen/stable/index.html)と統合して、マルチエージェントアプリケーションの実行をトレースおよび可視化するのに役立ちます。Weaveを初期化するだけで、`autogen_agentchat`、`autogen_core`、および`autogen_ext`内のインタラクションを自動的に追跡できます。このガイドでは、WeaveをAutoGenで使用するさまざまな例を紹介します。

## 前提条件

始める前に、AutoGenとWeaveがインストールされていることを確認してください。また、使用予定のLLMプロバイダー（OpenAI、Anthropicなど）のSDKも必要です。
...
スクリプトの先頭でWeaveを初期化して、トレースのキャプチャを開始します。
...
WeaveはAutoGen内のモデルクライアントに直接行われた呼び出しをトレースできます。

### クライアント作成呼び出しのトレース

この例では、`OpenAIChatCompletionClient`への単一の呼び出しをトレースする方法を示しています。
...
### ストリーミングを使用したクライアント作成呼び出しのトレース

Weaveはストリーミングレスポンスのトレースもサポートしています。
...
### Weaveはキャッシュされた呼び出しを記録します

AutoGenの`ChatCompletionCache`を使用でき、Weaveはこれらのインタラクションをトレースし、レスポンスがキャッシュから来たのか新しい呼び出しから来たのかを表示します。
...
## ツール呼び出しを使用したエージェントのトレース

Weaveはエージェントとそのツールの使用をトレースし、エージェントがどのようにツールを選択して実行するかの可視性を提供します。
...
## GroupChat - RoundRobinのトレース

グループチャット内のインタラクション（`RoundRobinGroupChat`など）はWeaveによってトレースされ、エージェント間の会話の流れを追跡できます。
...
AutoGenのメモリコンポーネントはWeaveでトレースできます。`@weave.op()`を使用して、より読みやすくするために単一のトレースの下にメモリ操作をグループ化できます。
...
検索拡張生成（RAG）ワークフロー（ドキュメントのインデックス作成と`ChromaDBVectorMemory`のようなメモリシステムを使用した検索を含む）はトレース可能です。RAGプロセスを`@weave.op()`でデコレートすることで、フロー全体を視覚化するのに役立ちます。

<Note>
  RAGの例には`chromadb`が必要です。`pip install chromadb`でインストールしてください。
</Note>
...
WeaveはAutoGenのエージェントランタイム（`SingleThreadedAgentRuntime`など）内の操作をトレースできます。`@weave.op()`をランタイム実行関数の周りに使用すると、関連するトレースをグループ化できます。
...
## ワークフロー（シーケンシャル）のトレース

エージェントインタラクションのシーケンスを定義する複雑なエージェントワークフローをトレースできます。`@weave.op()`を使用して、ワークフロー全体の高レベルトレースを提供できます。
...
<Warning>
  **Dockerが必要**
  この例ではDockerを使用したコード実行が含まれており、すべての環境（例：Colab直接）では動作しない場合があります。これを試す場合は、ローカルでDockerが実行されていることを確認してください。
</Warning>
...
* **Weave**：
  * [トレースガイド](/ja/guides/tracking/tracing)
  * [Opデコレータ](/ja/guides/tracking/ops)
* **AutoGen**：
