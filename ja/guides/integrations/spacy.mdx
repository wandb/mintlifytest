...
1. `WANDB_API_KEY` [環境変数](/ja/guides/track/environment-variables/) をあなたのAPIキーに設定します。
...
| `model_log_interval`   | `Optional int`型。デフォルトは`None`です。設定すると、[モデルのバージョン管理](/ja/guides/core/registry/)が[Artifacts](/ja/guides/artifacts/)とともに有効になります。モデルチェックポイントをログに記録する間隔のステップ数を渡します。デフォルトは`None`です。 |
...
トレーニングが始まると、トレーニングrun の[W&Bページ](/ja/guides/runs/)へのリンクが出力され、このrun の実験管理[ダッシュボード](/ja/guides/track/workspaces/)にWeights & BiasesのウェブUIでアクセスできます。