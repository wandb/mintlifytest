...
<Note>
上記のコードスニペットとこのページにリンクされている colab は、W&B CLI でスイープを初期化し作成する方法を示しています。W&B Python SDK コマンドを使用してスイープ設定を定義し、スイープを初期化・開始する手順については Sweeps の[ウォークスルー](/ja/guides/sweeps/walkthrough/)をご覧ください。
</Note>
...
* スイープ設定を定義し、スイープを初期化・開始するための W&B Python SDK コマンドの手順については、[スイープのウォークスルー](/ja/guides/sweeps/walkthrough/)をお読みください。
* 次のチャプターを探索して以下を学びます:
  * [W&B をコードに追加する](/ja/guides/sweeps/add-w-and-b-to-your-code/)
  * [スイープ設定を定義する](/ja/guides/sweeps/define-sweep-configuration/)
  * [スイープを初期化する](/ja/guides/sweeps/initialize-sweeps/)
  * [スイープエージェントを開始する](/ja/guides/sweeps/start-sweep-agents/)
  * [スイープ結果を視覚化する](/ja/guides/sweeps/visualize-sweep-results/)
* W&B Sweeps を使用したハイパーパラメータ最適化を探索する [スイープ実験の厳選リスト](/ja/guides/sweeps/useful-resources/)を探索します。結果は W&B Reports に保存されます。
