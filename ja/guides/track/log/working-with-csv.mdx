...
W&B Python ライブラリを使用して、CSV ファイルをログし、[W&B ダッシュボード](/ja/guides/track/workspaces/)で可視化します。W&B ダッシュボードは、機械学習モデルからの結果を整理し可視化する中心的な場所です。これは、W&B にログされていない[以前の機械学習実験の情報を含む CSV ファイル](#import-and-log-your-csv-of-experiments)や[データセットを含む CSV ファイル](#import-and-log-your-dataset-csv-file)がある場合に特に便利です。
...
2. CSV ファイルを W&B Table に変換し、[W&B ダッシュボード](/ja/guides/track/workspaces/)を利用します。
...
```
W&B Artifacts についての詳細は、[Artifacts チャプター](/ja/guides/artifacts/)を参照してください。
...
* 実験 run の名前
* 初期の[ノート](/ja/guides/runs/#add-a-note-to-a-run)
* 実験を区別するための[タグ](/ja/guides/runs/tags/)
* 実験に必要な設定（[Sweeps ハイパーパラメータチューニング](/ja/guides/sweeps/)の利用の利点があります）。
...
サマリーメトリクスの詳細については、[Log Summary Metrics](/ja/guides/track/log/log-summary/)を参照してください。

以下は、上記のサンプルテーブルを [W&B ダッシュボード](/ja/guides/track/workspaces/)に変換する完全な例のスクリプトです:
