...
<Note>
* W&B SDK / CLI / UI とあなたのバケットとの通信は、[事前署名URL](/ja/guides/hosting/data-security/presigned-urls/)を使用して行われます。
* W&B はガベージコレクションプロセスを使用して W&B Artifacts を削除します。詳細は[Artifacts の削除](/ja/guides/artifacts/delete-artifacts/)をご覧ください。
...
2. S3 バケットのプロビジョニング
    ...
    5. W&B プラットフォームのホスティング先の AWS アカウントに必要な S3 権限を付与します。これにより、AI ワークロードがクラウドインフラストラクチャーやユーザーブラウザーでバケットにアクセスするための[事前署名された URL](/ja/guides/hosting/data-security/presigned-urls/)が生成されます。
