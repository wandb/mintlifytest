...
- description: W&B Weave でスコアラーをガードレールとして使用する方法を学ぶ
---
...
Weave スコアラーは `score` メソッドを持つ特別なクラスで、呼び出しのパフォーマンスを評価できます。これらは非常にシンプルなルールから、判断者としての複雑なLLMまで多岐にわたります。

このノートブックでは、スコアラーをガードレールとして使用して、LLMが有害または不適切なコンテンツを生成するのを防ぐ方法を探ります。
