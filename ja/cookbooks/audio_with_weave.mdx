---
- title: Weaveでの音声の使用
- description: W&B Weaveで音声をWeaveで使用する方法を学びましょう
---

<Note>
  これはインタラクティブなノートブックです。ローカルで実行するか、以下のリンクを使用できます：

  * [Google Colabで開く](https://colab.research.google.com/github/wandb/weave/blob/master/docs/notebooks/audio_with_weave.ipynb)
  * [GitHubでソースを表示](https://github.com/wandb/weave/blob/master/docs/notebooks/audio_with_weave.ipynb)
</Note>
...
高度なユースケースでは、OpenAI Realtime APIを活用してリアルタイムで音声をストリーミングします。以下のサムネイルをクリックしてビデオデモを表示するか、[ここ](https://www.youtube.com/watch?v=lnnd73xDElw)をクリックしてください。
...
OpenAI（`openai`）とWeave（`weave`）の依存関係、およびAPIキー管理の依存関係`set-env`をインストールすることから始めます。
...
次に、OpenAIとWeaveに必要なAPIキーを読み込みます。ここでは、Google Colabのシークレットキーマネージャーと互換性のあるset\_envを使用します。これはColabの特定の`google.colab.userdata`の代替です。参照：[ここ](https://pypi.org/project/set-env-colab-kaggle-dotenv/)で使用方法の説明を確認できます。
...
次に、OpenAIの完了リクエストを定義し、Weaveデコレータ（op）を追加します。

ここでは、関数`prompt_endpont_and_log_trace`を定義します。この関数には主に3つのステップがあります：

1. テキストと音声の入出力をサポートする`GPT 4o Audio Preview`モデルを使用して完了オブジェクトを作成します。
...
以下のセルを実行します。システムとユーザープロンプトは出力音声とともにWeaveトレースに保存されます。
セルを実行した後、「🍩」絵文字の横にあるリンクをクリックしてトレースを表示します。
...
<details>
  <summary>（高度）WeaveによるリアルタイムオーディオAPI</summary>
  OpenAIのリアルタイムAPIは、リアルタイムの音声およびテキストアシスタントを構築するための高機能で信頼性の高い会話APIです。

  注意事項：

  * 以下のセルを確認してください：[マイク設定](#microphone-configuration)
  * Google Colab実行環境の制限により、**これはホストマシン上で実行する必要があります**（Jupyter Notebookとして）。ブラウザでは実行できません。
    * MacOSでは、`portaudio`をBrew経由でインストールする必要があります（[ここ](https://formulae.brew.sh/formula/portaudio)を参照）。これはPyaudioが機能するために必要です。
  * OpenAIのPython SDKはまだリアルタイムAPIをサポートしていません。より読みやすくするために、OAIリアルタイムAPIスキーマ全体をPydanticで実装しています。公式サポートがリリースされたら廃止される可能性があります。
  * この`enable_audio_playback`トグルを有効にすると、アシスタントが出力した音声が再生されます。このオプションを有効にする場合は、**ヘッドフォンが必要です**。エコー検出には非常に複雑な実装が必要なためです。
  ...
  以下のセルを実行して、利用可能なすべてのオーディオデバイスを確認します。次に、リストされたデバイスに基づいて`INPUT_DEVICE_INDEX`と`OUTPUT_DEVICE_INDEX`を設定します。入力デバイスには少なくとも1つの入力チャンネルがあり、出力デバイスには少なくとも1つの出力チャンネルがあります。
  ...
  ## OpenAI リアルタイムAPI スキーマの実装

  OpenAI Python SDKはまだリアルタイムAPIをサポートしていません。より読みやすくするために、OAIリアルタイムAPIスキーマ全体をPydanticで実装しています。公式サポートがリリースされたら廃止される可能性があります。

  <details>
    <summary>OpenAI リアルタイムAPI用のPydanticスキーマ（OpenAIのSDKにはリアルタイムAPIサポートがありません）</summary>
  ...
  リアルタイム（RT）オーディオモデルはウェブソケットを使用して、OpenAIのリアルタイムオーディオAPIにイベントを送信します。これは次のように機能します：

  1. **init:**&#x30ED;ーカルバッファ（入力オーディオ）とストリーム（アシスタント再生ストリーム、ユーザーオーディオディスクライターストリーム）を初期化し、リアルタイムAPIへの接続を開きます。
  2. **receive\_messages\_thread**：スレッドがAPIからのメッセージの受信を処理します。主に4つのイベントタイプが処理されます：- RESPONSE\_AUDIO\_TRANSCRIPT\_DONE：

     サーバーがアシスタントの応答が完了したことを示し、トランスクリプトを提供します。

     * CONVERSATION\_ITEM\_INPUT\_AUDIO\_TRANSCRIPTION\_COMPLETED：

       サーバーがユーザーの音声が文字起こしされたことを示し、ユーザーの音声のトランスクリプトを送信します。トランスクリプトをWeaveに記録し、ユーザーに表示します。

     * RESPONSE\_AUDIO\_DELTA：

       サーバーがアシスタント応答音声の新しいチャンクを送信します。これをレスポンスIDを介して進行中のレスポンスデータに追加し、再生用の出力ストリームに追加します。

     * RESPONSE\_DONE：

       サーバーがアシスタント応答の完了を示します。レスポンスに関連するすべての音声チャンクとトランスクリプトを取得し、Weaveに記録します。

     3.**send\_audio**：ハンドラーがユーザーの音声チャンクをバッファに追加し、音声バッファが一定のサイズに達したときに音声チャンクを送信します。
  ...
  RTAudioモデルの`send_audio`メソッドにリンクされたハンドラーを持つpyaudio入力ストリームを使用します。プログラム完了時に安全に終了できるように、ストリームはメインスレッドに返されます。
  ...
  メインスレッドはWeaveを統合したリアルタイムオーディオモデルを開始します。次に、録音が開始され、ユーザーからのキーボード割り込みを待ちます。
