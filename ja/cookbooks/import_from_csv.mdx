---
- title: CSVからインポート
- description: W&B WeaveでCSVからのインポートを使用する方法を学ぶ
---
...
場合によっては、WeaveのシンプルなインテグレーションでPythonやJavascriptコードを計装して、GenAIアプリケーションのリアルタイムトレースを取得することができないことがあります。多くの場合、これらのトレースは後で`csv`または`json`形式で利用可能になります。
...
このクックブックでのインポートの決定を理解するには、Weaveトレースには1対多の継続的な親子関係があることを知っておく必要があります。つまり、1つの親は複数の子を持つことができますが、その親自体が別の親の子である場合もあります。

したがって、`conversation_id`を親識別子として、`turn_index`を子識別子として使用して、完全な会話ログを提供します。
...
colabにアップロードするファイルの名前を`name_of_file`に設定し、これをログに記録するW\&Bのプロジェクトを`name_of_wandb_project`に設定します。
...
* Weave callはWeave traceと同等で、このcallには関連する親または子がある場合があります
...
評価についてさらに詳しく知るには、[クイックスタート](https://weave-docs.wandb.ai/tutorial-rag)をチェックして、新しく作成したデータセットを使用してRAGアプリケーションを評価しましょう！
