...
In this example, this tutorial shows an example that has a retrieval step to get documents. By tracking this, you can debug your app and see what documents were pulled into the LLM context.
This tutorial also shows how to evaluate it using an LLM judge.
...
First, compute the embeddings for your articles. You would typically do this once with your articles and put the embeddings & metadata in a database, but here the code does it every time the script runs for simplicity.
...
Next, wrap the retrieval function `get_most_relevant_document` with a `weave.op()` decorator and create the `Model` class. Call `weave.init('rag-qa')` to begin tracking all the inputs and outputs of the functions for later inspection.
...
As shown in the [Build an Evaluation pipeline tutorial](/tutorial-eval), you'll define a set of example rows to test your app against and a scoring function. The scoring function will take one row and evaluate it. The input arguments should match with the corresponding keys in the row, so `question` here will be taken from the row dictionary. `output` is the output of the model. The input to the model will be taken from the example based on its input argument, so `question` here too. The example uses `async` functions so they run fast in parallel. If you need a quick introduction to async, you can find one [here](https://docs.python.org/3/library/asyncio.html).
...
2. Overwrite the `score` function and add a `@weave.op()` if you want to track each call of the function
   - this function has to define an `output` argument where the prediction of the model will be passed to. Define it as type `Optional[dict]` in case the mode might return "None".
...
You've learned how to build observability into different steps of your applications, like the retrieval step in this example.
You've also learned how to build more complex scoring functions, like an LLM judge, for doing automatic evaluation of application responses.