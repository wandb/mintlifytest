...
- Registry admins can define and assign [*protected aliases*](/guides/registry/model_registry/access_controls/#add-protected-aliases) to represent key stages of your development pipeline. A protected alias can be assigned only by a registry admin. W&B blocks other users from adding or removing protected aliases from versions in a registry using the API or UI.
- You can now filter console logs based on a run's `x_label` value. During [distributed training](/guides/track/log/distributed-training/#track-all-processes-to-a-single-run), this optional parameter tracks the node that logged the run.
...
- You can now slide through [media panels](/guides/app/features/panels/media/) using non-`_step` fields such as `epoch` or `train/global_step` (or anything else).
- In Tables and plots in [Query Panels](/guides/app/features/panels/query-panels/) that use `runs` or `runs.history` expressions, a step slider allows you to step through the progress on your metrics, text, or media through the course of your runs. The slider supports stepping through  non-`_step` metrics.
- You can now customize [bar chart](/guides/app/features/panels/bar-plot/) labels using a font size control.
...
- **Personal workspace templates** allow you to save your workspace setup so it is automatically applied to your new [projects](/guides/track/project-page/). Initially, you can configure certain line plot settings such as the default X axis metric, smoothing algorithm, and smoothing factor.
- **Improved Exponentially-weighted Moving Average (EMA) smoothing** provides more reliable [smoothed lines](/guides/app/features/panels/line-plot/smoothing/) when operating on complete, unbinned data. In most cases, smoothing is handled at the back end for improved performance.
...
## Security
- Registry admins can now designate a [service account](/guides/hosting/iam/service-accounts/) in a registry as either a Registry Admin or a Member. Previously, the service account’s role was always Registry Admin. See [Registry service account configuration](/guides/registry/configure_registry/).
...
- Improved Fork and Rewind Performance. 
    ...
    [Forking](/guides/runs/forking/) a run creates a new run that uses the same configuration as an existing run. Changes to the forked run do not the parent run, and vice versa. A pointer is maintained between the forked run and the parent. [Rewinding](/guides/runs/rewind/) a run lets you log new data from that point in time without losing the existing data.
